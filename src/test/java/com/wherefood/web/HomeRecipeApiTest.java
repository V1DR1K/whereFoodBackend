...
class HomeRecipeApiTest {
 ...
 void letsAnyAuthenticatedUserUpdateACooking() {
  ...
  when(cookings.findDetailedById(2L)).thenReturn(Optional.of(cooking)); when(cookings.save(cooking)).thenReturn(cooking); when(photos.findByCookingIdOrderByPositionAscIdAsc(2L)).thenReturn(List.of()); when(reviews.findByCookingIdOrderByAuthorUsername(2L)).thenReturn(List.of());
   ...
   CookingDto result = new HomeRecipeApi(recipes, mock(RecipePhotos.class), cookings, photos, reviews, null).updateCooking(2L, new CookingRequest(Home.AVRIL, 4, LocalDate.of(2026, 7, 21), MealType.ALMUERZO), avril);
 ...
 void createsAReusableRecipeDefinition() {
  Recipes recipes = mock(Recipes.class); User tomas = user(7L, "tomas"); when(recipes.save(any(Recipe.class))).thenAnswer(invocation -> { Recipe value = invocation.getArgument(0); value.id = 5L; return value; });
   ...
   RecipeDto result = new HomeRecipeApi(recipes, mock(RecipePhotos.class), null, null, null, null).addRecipe(new RecipeRequest("Tarta", "https://example.test/tarta", List.of(new RecipeIngredientRequest("Harina", BigDecimal.valueOf(250), "g")), List.of(new RecipeStepRequest("Hornear."))), tomas);
 ...
 void listsRecipesWithIngredientsAndStepsUsingIndexedCollections() throws NoSuchFieldException {
  ...
  when(recipes.findAll()).thenReturn(List.of(recipe));
   ...
   List<RecipeDto> result = new HomeRecipeApi(recipes, mock(RecipePhotos.class), null, null, null, null).listRecipes(null);
  ...
  @Test
  void projectsTheRecipeProfileSeparatelyFromCookingPhotos() {
   ...
   RecipeDto result = new HomeRecipeApi(recipes, profilePhotos, null, null, null, null).listRecipes(null).getFirst();
