...
public class WhyFunApi {
 ...
 @PostMapping("/plans") @ResponseStatus(HttpStatus.CREATED) @Transactional FunPlanDto addPlan(@RequestBody @Valid FunPlanRequest request, @AuthenticationPrincipal User author) { WhyFunVenue plan = new WhyFunVenue(); plan.createdBy = author; apply(plan, request); plan.createdAt = plan.updatedAt = Instant.now(); return plan(venues.save(plan)); }
 @PutMapping("/plans/{id}") @Transactional FunPlanDto updatePlan(@PathVariable Long id, @RequestBody @Valid FunPlanRequest request, @AuthenticationPrincipal User author) { WhyFunVenue plan = owned(findPlan(id), author); apply(plan, request); plan.updatedAt = Instant.now(); return plan(venues.save(plan)); }
 @DeleteMapping("/plans/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) @Transactional void deletePlan(@PathVariable Long id, @AuthenticationPrincipal User author) { venues.delete(owned(findPlan(id), author)); }
 @PostMapping(value = "/plans/{id}/photos", consumes = MediaType.MULTIPART_FORM_DATA_VALUE) @Transactional FunPlanDto uploadPhoto(@PathVariable Long id, @RequestPart("file") MultipartFile file, @AuthenticationPrincipal User author) throws IOException { WhyFunVenue plan = owned(findPlan(id), author); if (photos.countByVenueId(id) >= MAX_PHOTOS) throw conflict("Cada plan admite hasta " + MAX_PHOTOS + " fotos"); WhyFunVenuePhoto photo = photos.save(storage.store(plan, file)); if (plan.coverPhotoId == null) { plan.coverPhotoId = photo.id; venues.save(plan); } return plan(plan); }
 @PutMapping("/plans/{id}/cover/{photoId}") @Transactional FunPlanDto setCover(@PathVariable Long id, @PathVariable Long photoId, @AuthenticationPrincipal User author) { WhyFunVenue plan = owned(findPlan(id), author); WhyFunVenuePhoto photo = photos.findDetailedById(photoId).orElseThrow(() -> notFound("Foto")); if (!photo.venue.id.equals(plan.id)) throw badRequest("La foto no pertenece a este plan"); plan.coverPhotoId = photo.id; return plan(venues.save(plan)); }
 @DeleteMapping("/photos/{photoId}") @ResponseStatus(HttpStatus.NO_CONTENT) @Transactional void deletePhoto(@PathVariable Long photoId, @AuthenticationPrincipal User author) { WhyFunVenuePhoto photo = photos.findDetailedById(photoId).orElseThrow(() -> notFound("Foto")); WhyFunVenue plan = owned(photo.venue, author); if (photo.id.equals(plan.coverPhotoId)) { plan.coverPhotoId = null; venues.save(plan); } photos.delete(photo); }
 ...
 @PutMapping("/plans/{id}/review") @Transactional FunReviewDto saveReview(@PathVariable Long id, @RequestBody @Valid FunReviewRequest request, @AuthenticationPrincipal User author) { requireCoupleReviewer(author); WhyFunVenue plan = findPlan(id); if (plan.scheduledAt == null || plan.scheduledAt.isAfter(LocalDateTime.now())) throw badRequest("Podés opinar después de que ocurra la salida"); WhyFunVenueReview review = reviews.findByVenueIdAndAuthorId(id, author.id).orElseGet(() -> { WhyFunVenueReview value = new WhyFunVenueReview(); value.venue = plan; value.author = author; value.createdAt = Instant.now(); return value; }); review.rating = request.rating(); review.comment = blankToNull(request.comment()); review.updatedAt = Instant.now(); return review(reviews.save(review)); }
